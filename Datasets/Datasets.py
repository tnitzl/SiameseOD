...
class Siamese_Dataset(Dataset):
    def __init__(self, anomaly: np.ndarray, normal: np.ndarray) -> None:
        ...
        print(
            f"Der Siamese Datasatz wurde mit einer gesamtLänge von {len(self.data)} erstellt"
        )
...
class Random_Dataset(Dataset):
    def __init__(
        ...
        x: np.ndarray,
        y: np.ndarray,
        percent_labeld: float,
        len_dataset: int = 10000,
    ):
        ...
        random_indicies_anomaly = np.random.choice(
            anomaly.shape[0], num_samples_anomaly, replace=False
        )
        self.labeld_anoamly = anomaly[random_indicies_anomaly]
        random_indicies_normal = np.random.choice(
            normal.shape[0], num_samples_normal, replace=False
        )
        self.labeld_normal = normal[random_indicies_normal]

        all_indicies_anomaly = np.arange(anomaly.shape[0])
        unlabeld_indicies_anomaly = np.setdiff1d(
            all_indicies_anomaly, random_indicies_anomaly
        )
        self.unlabeld_anomaly = anomaly[unlabeld_indicies_anomaly]

        all_indicies_normal = np.arange(normal.shape[0])
        unlabeld_indicies_normal = np.setdiff1d(
            all_indicies_normal, random_indicies_normal
        )
    ...
    def __getitem__(self, idx) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if zufallszahl < 2:
            ...
            while second_random_index == first_random_index:
                second_random_index = random.randint(0, len(self.labeld_anoamly) - 1)
            out1 = torch.tensor(
                self.labeld_anoamly[first_random_index], dtype=torch.float32
            )
            out2 = torch.tensor(
                self.labeld_anoamly[second_random_index], dtype=torch.float32
            )
        ...
        # labeld normal - normal
            ...
            while second_random_index == first_random_index:
                second_random_index = random.randint(0, len(self.labeld_normal) - 1)
            out1 = torch.tensor(
                self.labeld_normal[first_random_index], dtype=torch.float32
            )
            out2 = torch.tensor(
                self.labeld_normal[second_random_index], dtype=torch.float32
            )
            lable = torch.tensor(1, dtype=torch.float32)
            return out1, out2, lable
    ...
    def get_complete_dataset(self) -> Simple_Dataset:
        data = np.append(self.unlabeld_normal, self.unlabeld_anomaly, axis=0)
        data = np.append(data, self.labeld_anoamly, axis=0)
        data = np.append(data, self.labeld_normal, axis=0)
        label = (
            [-1] * len(self.unlabeld_normal)
            + [-1] * len(self.unlabeld_anomaly)
            + [1] * len(self.labeld_anoamly)
            + [0] * len(self.labeld_normal)
        )
        return Simple_Dataset(data=data, label=label)
...
class Base_Dataset:
    def __init__(self, X: np.ndarray, y: np.ndarray, percent_labeld: float):
        ...
        print(
            f"Die Länge das Anomalydatensatzen ist {len(anomaly)} und der normalen daten ist: {len(normal)}"
        )
        print(f"Es werden {percent_labeld * 100}% der Daten gelabeld")
        print(
            f"Es wurden zwei Datensätze erstellt, der erste mit der beiden Längen {len(self.labeld_anoamly)} und {len(self.labeld_normal)}"
        )
        print(
            f"Die ungelabelden parts dazu sind {len(self.unlabeld_anomaly)} und {len(self.unlabeld_normal)}"
        )
